...
let asset_const=()=>{ 
   ...
   ajaxcall.then((recv)=>{
      ...
      /// main
      /// 
      /// テンプレートからカードを作成(1毎づつ作成してreltの下に追加)
      /// <div class="card">にdatasetとして assetid(資産番号)を追加しておく
      ...
      for(let i=0;i<recv.length;i++){
         ...
         { // <div class="card">
            ...
            items[0].setAttribute("data-assetid",recv[i]["assetid"]);
            items[0].setAttribute("id","asset_id_"+recv[i]["assetid"]);
         }
         /// カードを作成
         { //// 画像
         ...
         { //// スイッチ
            ...
            if(recv[i]["rel"]==="true"){items[0].checked=true;}else{items[0].checked=false;}
            if(recv[i]["repsw"]==="true"){items[1].checked=true;}else{items[1].checked=false;}
         }
         { //// 資産情報
